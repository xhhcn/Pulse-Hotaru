...
<p align="center">
  <b>Pulse 的 Hotaru 主题拓展发行版</b><br>
  保持 Pulse 后端兼容与数据兼容，提供更贴近 Hotaru 的前端体验
</p>
...
- 🎨 **Hotaru 风格前端** - 首页 / 管理 / 登录三页统一为 Hotaru 视觉体系
- 🔁 **与 Pulse 数据兼容** - 直接复用现有 `metrics.db`，无需迁移数据库
- 🔧 **发布分离** - 独立仓库与镜像（`Pulse-Hotaru` / `xhh1128/pulse-hotaru`），不覆盖原 Pulse 发行
...
### 从 Pulse 无损切换到 Pulse-Hotaru（保留原数据）

如果你已经在运行 Pulse，并且希望只替换为 Hotaru 主题版，同时保留所有历史数据（系统列表、管理员密码、TCPing 历史、共享密钥等），可以直接覆盖服务端二进制：

> 下面命令 **不会删除** `/opt/pulse/data/metrics.db`，只替换程序本体。
...
客户端无需重装，继续使用 Pulse 官方客户端安装脚本即可：
```bash
...
│   ├── styles/hotaru-theme.css   # Hotaru 主样式（首页 + 管理 + 登录）
│   └── styles/global.css         # 全局 Tailwind 基础样式
├── tailwind.config.mjs           # Tailwind 配置（项目构建依赖）
...
```bash
git clone https://github.com/<你的用户名>/Pulse-Hotaru.git
